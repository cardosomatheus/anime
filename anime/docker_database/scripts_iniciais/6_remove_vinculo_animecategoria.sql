-- DROP PROCEDURE public.proc_rem_anime_categoria(int8, int4);

CREATE OR REPLACE PROCEDURE public.proc_rem_anime_categoria(IN p_id_anime bigint, IN p_id_categoria integer)
 LANGUAGE plpgsql
AS $procedure$   
DECLARE
...
BEGIN
    ...
    IF IND_CATEGORIA_ANIME < 1 THEN
        RAISE EXCEPTION 'Não existe essa categoria no anime desejado.';
    END IF;


    -- 2º Insere o vinculo entre ambos.
    DELETE 
        FROM TB_ANIME_CATEGORIA AC
      WHERE AC.ID_CATEGORIA = P_ID_CATEGORIA
        AND AC.ID_ANIME = P_ID_ANIME;
